...
public class JEditBuffer {
	...
	Breakpoints breakpoints = new Breakpoints(this); // see #1603.
	...
	public JEditBuffer(Map props) {
		...
		for (Map.Entry entry : set) {
			properties.put(entry.getKey(), new PropValue(entry.getValue(),
					false));
		} // }}}
		...
		if (getProperty(ENCODING) == null)
			properties.put(ENCODING,
					new PropValue(System.getProperty("file.encoding"), false));
		if (getProperty(LINESEP) == null)
			properties.put(LINESEP,
					new PropValue(System.getProperty("line.separator"), false));
	} // }}}
	...
	public void remove(int offset, int length) {
		...
		try {
			...
			if (!undoInProgress && !loading) {
				undoMgr.contentRemoved(offset, length, getText(offset, length),
						!dirty);
			}
	...
	public void shiftIndentLeft(int[] lines) {
		...
		try {
			...
			for (int i = 0; i < lines.length; i++) {
				...
				if (whiteSpace == 0)
					continue;
				int whiteSpaceWidth = Math.max(
						0,
						StandardUtilities.getLeadingWhiteSpaceWidth(line,
								tabSize) - indentSize);

				insert(lineStart + whiteSpace,
						StandardUtilities.createWhiteSpace(whiteSpaceWidth,
								noTabs ? 0 : tabSize));
	...
	public boolean indentLine(int lineIndex, boolean canDecreaseIndent) {
		...
		try {
			...
			insert(start, StandardUtilities.createWhiteSpace(idealIndent,
	...
	public int getIdealIndentForLine(int lineIndex) {
		...
		int oldIndent = prevLineIndex == -1 ? 0 : StandardUtilities
				.getLeadingWhiteSpaceWidth(getLineSegment(prevLineIndex),
						getTabSize());
		...
		for (IndentAction action : actions) {
			newIndent = action.calculateIndent(this, lineIndex, oldIndent,
					newIndent);
	...
	public void insertAtColumn(int line, int col, String str) {
		try {
			...
			if (offset == -1) {
				...
				str = StandardUtilities.createWhiteSpace(col - total[0], 0)
						+ str;
			} else
	...
	public int insertIndented(int offset, String text) {
		try {
			...
			int leadingIndent = StandardUtilities.getLeadingWhiteSpaceWidth(
					lineText, getTabSize());

			String whiteSpace = StandardUtilities.createWhiteSpace(
					leadingIndent, getBooleanProperty("noTabs") ? 0
							: getTabSize());
	...
	public void markTokens(int lineIndex, TokenHandler tokenHandler) {
		...
		for (int i = start; i <= lineIndex; i++) {
			...
			TokenMarker.LineContext prevContext = ((i == 0 || textMode) ? null
					: lineMgr.getLineContext(i - 1));

			context = tokenMarker
					.markTokens(prevContext, (i == lineIndex ? tokenHandler
	...
	public void propertiesChanged() {
		...
		if (handler != null) {
			...
			if (folding != null)
				Log.log(Log.WARNING, this, "invalid 'folding' property: "
						+ folding);
			setFoldHandler(new DummyFoldHandler());
		}
	} // }}}
	...
	public int getIntegerProperty(String name, int defaultValue) {
		...
		if (obj == null)
			...
			try {
				...
				properties.put(name, new PropValue(returnValue,
						defaultValueFlag));
				return returnValue;
			} catch (Exception e) {
				return defaultValue;
	...
	public ParserRuleSet getRuleSetAtOffset(int offset) {
		...
		Token token = TextUtilities
				.getTokenAtOffset(tokens.getTokens(), offset);
		return token.rules;
	} // }}}
	...
	public BufferListener[] getBufferListeners() {
		BufferListener[] returnValue = new BufferListener[bufferListeners
				.size()];
		...
		for (int i = 0; i < bufferListeners.size(); i++) {
			...
			try {
				...
				Log.log(Log.ERROR, this,
						"Exception while sending buffer event to " + listener
								+ " :");
	...
	protected void fireContentInserted(int startLine, int offset, int numLines,
			int length) {
		for (int i = 0; i < bufferListeners.size(); i++) {
			BufferListener listener = getListener(i);
			try {
				listener.contentInserted(this, startLine, offset, numLines,
						length);
			} catch (Throwable t) {
				Log.log(Log.ERROR, this,
						"Exception while sending buffer event to " + listener
	...
	protected void fireContentRemoved(int startLine, int offset, int numLines,
			int length) {
		for (int i = 0; i < bufferListeners.size(); i++) {
			BufferListener listener = getListener(i);
			try {
				listener.contentRemoved(this, startLine, offset, numLines,
						length);
			} catch (Throwable t) {
				Log.log(Log.ERROR, this,
						"Exception while sending buffer event to " + listener
	...
	protected void firePreContentInserted(int startLine, int offset,
		...
		for (int i = 0; i < bufferListeners.size(); i++) {
			BufferListener listener = getListener(i);
			try {
				listener.preContentInserted(this, startLine, offset, numLines,
						length);
			} catch (Throwable t) {
				Log.log(Log.ERROR, this,
						"Exception while sending buffer event to " + listener
	...
	protected void firePreContentRemoved(int startLine, int offset,
		...
		for (int i = 0; i < bufferListeners.size(); i++) {
			BufferListener listener = getListener(i);
			try {
				listener.preContentRemoved(this, startLine, offset, numLines,
						length);
			} catch (Throwable t) {
				Log.log(Log.ERROR, this,
						"Exception while sending buffer event to " + listener
	...
	protected void loadText(Segment seg, IntegerArray endOffsets) {
		...
		try {
			...
			firePreContentInserted(0, 0, endOffsets.getSize() - 1,
					seg.count - 1);
	...
	protected void parseBufferLocalProperties() {
		...
		if (firstLine < getLineCount()) {
			...
			parseBufferLocalProperties(getSegment(
					getLineStartOffset(firstLine), length));
		}
	} // }}}
	...
	/** ----------filename support below Added Wouter 2dec08 ---------------- */
	/*
	 * Wouter: don't know why they incorporated these simple derivatives getName
	 * and getDirectory here
	 */
	...
	public String getName() {
		return new FavoritesVFS().getFileName(filepath);
	}

	public String getDirectory() {
		return new FavoritesVFS().getParentOfPath(filepath);

	}

	/** -------- breakpoint support below added W.Pasman 31mar11---------------- */
