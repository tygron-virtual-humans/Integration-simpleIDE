...
import org.gjt.sp.jedit.StandAloneBeanShellFacade;
...
public class SearchAndReplace {
	 ...
	 * 
	 * @param wrap
	 *            If true, the 'continue search from start' dialog will not be
	 *            displayed
	 * @since jEdit 3.2pre2
	 */
	 ...
	 * {@link #setSearchString(String)}, {@link #setIgnoreCase(boolean)}, or
	 * {@link #setRegexp(boolean)} will reset the matcher to the default.
	 */
	 ...
	 * 
	 * @return a SearchMatcher or null if there is no search or if the matcher
	 *         can match empty String
	 * 
	 * @exception IllegalArgumentException
	 *                if regular expression search is enabled, the search string
	 ...
	 * @since jEdit 2.7pre3
	 */
	public static boolean hyperSearch(ViewInterface view) {
	 ...
	 *            The view
	 * @param selection
	 *            If true, will only search in the current selection. Note that
	 *            the file set must be the current buffer file set for this to
	 *            work.
	 * @since jEdit 4.0pre1
	 */
	public static boolean hyperSearch(ViewInterface view, boolean selection) {
		throw new RuntimeException("hyperSearch not supported");
		// HACK we can't in GOAL, we don't have an results panel. W.Pasman
		// 29apr10

		/*
		 * // component that will parent any dialog boxes Component comp =
		 * SearchDialog.getSearchDialog(view); if(comp == null) comp =
		 * view.getTextArea();
		 * 
		 ...
		 * .getDockable(HyperSearchResults.NAME); results.searchStarted();
		 * 
		 * try { SearchMatcher matcher = getSearchMatcher(); if(matcher == null)
		 * { view.getFrame().getToolkit().beep(); results.searchFailed(); return
		 * false; }
		 * 
		 * Selection[] s; if(selection) { s = view.getTextArea().getSelection();
		 * if(s == null) { results.searchFailed(); return false; } } else s =
		 * null; VFSManager.runInWorkThread(new HyperSearchRequest(view,
		 * matcher,results,s)); return true; } catch(Exception e) {
		 * results.searchFailed(); handleError(comp,e); return false; }
		 */
	} // }}}
	 ...
	 */
	public static boolean find(ViewInterface view) {
		...
		try {
			// view.showWaitCursor(); // Wouter: turned off, no showWaitCursor
			// in ViewInterface now.
			...
			loop: for (;;) {
				while (path != null) {
					// Buffer buffer =
					// jEdit.openTemporary(view,null,path,false);
					JEditBuffer buffer = view.getBuffer(); // Wouter: hacked.

					/*
					 * this is stupid and misleading. but 'path' is not used
					 * anywhere except the above line, and if this is done after
					 * the 'continue', then we will either hang, or be forced to
					 * duplicate it inside the buffer == null, or add a
					 * 'finally' clause. you decide which one's worse.
					 */
					...
					// Wouter: disbaled, JEditBuffer is object in memory I
					// suppose.
					...
					if (view.getBuffer() == buffer && !repeat) {
						...
						Selection s = textArea.getSelectionAtOffset(textArea
								.getCaretPosition());
						if (s == null)
				...
				if (repeat) {
					if (!BeanShell.isScriptRunning()) {
						// view.getStatus().setMessageAndClear( //Wouter:
						// hacked. We dont have getStatus() now.
						// TODO get better error handling, reroute, whatever.
						// removed, #1250
						// System.out.println(jEdit.getProperty("view.status.search-not-found"));
				...
				if (wrap) {
					if (!BeanShell.isScriptRunning()) {
						// view.getStatus().setMessageAndClear( Wouter: HACK, we
						// dont have status bar...
						// removed, #1250
						// System.out.println(jEdit.getProperty("view.status.auto-wrap"));
					...
					int result = GUIUtilities.confirm(comp, "keepsearching",
			...
			// view.hideWaitCursor(); //Wouter: turned off.
		}
	 ...
	 */
	public static boolean find(ViewInterface view, Buffer buffer, int start)
			throws Exception {
	 ...
	 * @param firstTime
	 *            See
	 *            {@link SearchMatcher#nextMatch(CharSequence,boolean,boolean,boolean,boolean)}
	 *            .
	 * @since jEdit 4.1pre7
	 */
	public static boolean find(ViewInterface view, JEditBuffer buffer,
			int start, boolean firstTime, boolean reverse) throws Exception {

		// EditBus.send(new PositionChanging(view.getEditPane()));
		EditBus.send(new PositionChanging(view.getTextArea())); // Wouter:
																// hacked, no
																// idea if this
																// works.
		...
		SearchMatcher.Match match = matcher.nextMatch(text, startOfBuffer,
				endOfBuffer, firstTime, reverse);
		if (match != null) {
			// jEdit.commitTemporary(buffer); //Wouter: this is not possible in
			// the GOAL gui.
			// view.setBuffer(buffer,true); Wouter: we don't do multiple
			// buffers...
			TextArea textArea = view.getTextArea();

			if (reverse) {
				textArea.setSelection(new Selection.Range(start - match.end,
						start - match.start));
				// make sure end of match is visible
				textArea.scrollTo(start - match.start, false);
				textArea.moveCaretPosition(start - match.end);
			} else {
				textArea.setSelection(new Selection.Range(start + match.start,
						start + match.end));
				textArea.moveCaretPosition(start + match.end);
	 ...
	 * @return True if the operation was successful, false otherwise
	 */
	public static boolean replace(ViewInterface view) {
		// component that will parent any dialog boxes
		Component comp = SearchDialog.getSearchDialog(view);
		if (comp == null)
			comp = view.getTextArea();
		...
		try {
			...
			boolean _reverse = !regexp && reverse
			...
			if (!BeanShell.isScriptRunning()) {
				...
				// Wouter: changed, we don't have status bar.
				// view.getStatus().setMessageAndClear(
				// removed, #1250
				// System.out.println(jEdit.getProperty("view.status.replace-all",args));
			}
	 ...
	 * @return True if the operation was successful, false otherwise
	 */
	public static boolean replace(ViewInterface view, JEditBuffer buffer,
			int start, int end) {
		if (!buffer.isEditable())
			return false;
	 ...
	 */
	public static boolean replaceAll(ViewInterface view) {
	 ...
	 * @param view
	 *            The view
	 * @param dontOpenChangedFiles
	 *            Whether to open changed files or to autosave them quietly
	 * @return the number of modified files
	 */
	public static boolean replaceAll(ViewInterface view,
		...
		// view.showWaitCursor(); Wouter: disabled.

		boolean smartCaseReplace = (replace != null && TextUtilities
				.getStringCase(replace) == TextUtilities.LOWER_CASE);
		...
		try {
			...
			loop: while (path != null) {
				...
				/*
				 * this is stupid and misleading. but 'path' is not used
				 * anywhere except the above line, and if this is done after the
				 * 'continue', then we will either hang, or be forced to
				 * duplicate it inside the buffer == null, or add a 'finally'
				 * clause. you decide which one's worse.
				 */
				path = fileset.getNextFile(view, path);
				...
				try {
					buffer.beginCompoundEdit();
					retVal = _replace(view, buffer, matcher, 0,
							buffer.getLength(), smartCaseReplace);
				} finally {
					buffer.endCompoundEdit();
				}
				...
				if (retVal != 0) {
					...
					if (dontOpenChangedFiles) {
						// buffer.save(null,null); // Wouter: standalone should
						// not do anything with files..(?)
					} else {
						// jEdit.commitTemporary(buffer); // Wouter: we don't do
						// temporary buffers in search?
			...
			// view.hideWaitCursor(); Wouter we don't have this right now.
		}
	...
	private static NameSpace replaceNS = new NameSpace(
			BeanShell.getNameSpace(), BeanShell.getNameSpace()
					.getClassManager(), "search and replace");
	...
	// {{{ record() method
	private static void record(ViewInterface view, String action,
	...
	private static int replaceInSelection(ViewInterface view,
			TextArea textArea, JEditBuffer buffer, SearchMatcher matcher,
			boolean smartCaseReplace, Selection s) throws Exception {
		/*
		 * if an occurence occurs at the beginning of the selection, the
		 * selection start will get moved. this sucks, so we hack to avoid it.
		 */
		...
		if (s instanceof Selection.Range) {
			returnValue = _replace(view, buffer, matcher, s.getStart(),
					s.getEnd(), smartCaseReplace);
	 ...
	 */
	private static int _replace(ViewInterface view, JEditBuffer buffer,
			SearchMatcher matcher, int start, int end, boolean smartCaseReplace)
			throws Exception {
		int occurCount = 0;

		boolean endOfLine = (buffer.getLineEndOffset(buffer
				.getLineOfOffset(end)) - 1 == end);
		...
		loop: for (int counter = 0;; counter++) {
			...
			SearchMatcher.Match occur = matcher.nextMatch(text, startOfLine,
					endOfLine, counter == 0, false);
			if (occur == null)
				break loop;

			String found = text.subSequence(occur.start, occur.end).toString();

			int length = replaceOne(view, buffer, occur, offset, found,
					smartCaseReplace);
			if (length == -1)
				offset += occur.end;
			else {
	 ...
	 */
	private static int replaceOne(ViewInterface view, JEditBuffer buffer,
	...
	// {{{ replaceOne() method
	private static String replaceOne(ViewInterface view,
			SearchMatcher.Match occur, String found) throws Exception {
		if (regexp) {
	...
	private static String regexpBeanShellReplace(ViewInterface view,
		...
		// Wouter: fixed: now we need the Standalone version...
		BeanShellFacade<ViewInterface> bsh = new StandAloneBeanShellFacade();
		Object obj = bsh.runCachedBlock(replaceMethod, view, replaceNS);
		if (obj == null)
			return "";
		else
			return obj.toString();
	...
	private static String literalBeanShellReplace(ViewInterface view,
