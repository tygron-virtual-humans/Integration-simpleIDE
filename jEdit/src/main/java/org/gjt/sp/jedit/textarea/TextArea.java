...
public abstract class TextArea extends JComponent {
	 ...
	 * See {@link jEdit#createTextArea()} Wouter: added ViewInterface construct
	 * to handle higher-level action invocation
	 */
	public void initInputHandler(final ViewInterface view) {
	...
	// {{{ lineComment() method
	/**
	 * removes the line comment string from the start of each line of the
	 * selection. Ignores the line if the line does not start with the comment
	 * string. The function will only beep to indicate error but not replace any
	 * text when
	 * <ul>
	 * <li>buffer is not editable
	 * <li>the lineComment property is not set
	 * <li>the lineComment property is the empty string
	 * <li>the lineComment property contains whitespace(s)
	 * </ul>
	 * 
	 * Furthermore we handle layout as follows
	 * <ul>
	 * <li>If the line starts with whitespace, these whitespaces are untouched.
	 * <li>If there is a ' ' after the comment character, that is removed as
	 * well. Only the first ' ' will be removed.
	 * </ul>
	 * 
	 * @since 23may2011 W.Pasman
	 */
	public void lineUncomment() {
		...
		try {
			for (int i = 0; i < lines.length; i++) {
				String text = getLineText(lines[i]);
				int offset = StandardUtilities.getLeadingWhiteSpace(text);
				if (text.substring(offset).startsWith(comment)) {
					int toRemove = commentlength;
					if (text.substring(offset).startsWith(commentPlus)) {
						toRemove = commentlength + 1;
					}
					buffer.remove(getLineStartOffset(lines[i]) + offset,
							toRemove);
				}
			}
		} finally {
