 ...
 * @author Slava Pestov
 * @author W.Pasman hacked to fit ViewInterface 2dec08
...
public class SearchDialog extends EnhancedDialog implements EBComponent
{
	...
	public static SearchDialog getSearchDialog(ViewInterface view)
	{
	 ...
	 * @since jEdit 4.0pre6
	 */
	public static void showSearchDialog(ViewInterface view, String searchString,
		int searchIn)
	{
	...
	public void ok()
	{
		try
		{
			...
			{
				GUIUtilities.error(view.getFrame(),"search-no-selection",null);
				return;
			}
	...
	private static final Map<ViewInterface, SearchDialog> viewHash = new HashMap<ViewInterface, SearchDialog>();
	 ...
	 * @param view The view
	 */
	private SearchDialog(ViewInterface view)
	{
		super(view.getFrame(),jEdit.getProperty("search.title"),false);
		...
		public void actionPerformed(ActionEvent evt)
		{
			...
			else if(source == replaceAllBtn)
			{
				if(searchSelection.isSelected() &&
					...
					== 0)
				{
					GUIUtilities.error(view.getTextArea(),"search-no-selection",null);
					return;
				}
