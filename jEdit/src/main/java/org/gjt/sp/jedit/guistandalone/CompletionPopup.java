...
public class CompletionPopup extends JWindow
{
	 ...
	 */ 
	public CompletionPopup(ViewInterface view)
	{
		super(view.getFrame());
		this.view = view;
	...
	public CompletionPopup(ViewInterface view, Point location)
	{
		this(view);
		if (location != null)
		{
	...
	private void passKeyEventToView(KeyEvent e)
	{
		...
		// HACK Wouter: org.gjt.sp.jedit.View is a hack to access a constant used at more places...
		// this probably should be a more global constant, maybe inside ViewInterface or so...
		view.getInputHandler().processKeyEvent(e, org.gjt.sp.jedit.View.ACTION_BAR, false);
	...
	private class KeyHandler extends KeyAdapter
	{
		...
		public void keyPressed(KeyEvent e)
		{
			...
			if (candidates == null || !candidates.isValid())
			{
				...
				// Wouter:  if you use keybd shortcut instead of menu, you use Meta to enable completion popup
				// if we don't consume it, the popup will be disposed in default...
			else if (e.isMetaDown()) e.consume(); 
		...
		{
			// Wouter: throw away all meta-down events; they are used to control menus
			// and they don't generate proper keystrokes. 
			if (e.isMetaDown()) { e.consume(); return; }
