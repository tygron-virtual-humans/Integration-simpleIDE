 ...
 * 
 * Wouter: hacked version!!
 * 
 * Modified W.Pasman 27nov08 version of the gui.CompleteWord, 
 * to work with an ViewInterface instead of 
 * directly with a View.
 * That allows us to hook up other views instead of the jEdit one.
 *
...
package org.gjt.sp.jedit.guistandalone;
...
public class CompleteWord extends CompletionPopup {
	// {{{ completeWord() method
	public static void completeWord(ViewInterface view) {
		TextArea textArea = view.getTextArea(); // Wouter: changed from
												// jEditTextArea.
		...
		// {{{ if there is only one competion, insert in buffer
			...
			String longestPrefix = MiscUtilities.getLongestPrefix(completions,
					keywordMap != null ? keywordMap.getIgnoreCase() : false);
			...
			SwingUtilities
					.convertPointToScreen(location, textArea.getPainter());
			new CompleteWord(view, longestPrefix, completions, location,
					noWordSep);
		} // }}}
	} // }}}

	// {{{ CompleteWord constructor
	public CompleteWord(ViewInterface view, String word,
			Completion[] completions, Point location, String noWordSep) {
		super(view, location);
	...
	private static String getNonAlphaNumericWordChars(JEditBuffer buffer,
	...
	private static String getWordToComplete(JEditBuffer buffer, int caretLine,
	...
	private static Completion[] getCompletions(final JEditBuffer buffer,
			final String word, final int caret) {
		// build a list of unique words in all visible buffers
		final Set<Completion> completions = new TreeSet<Completion>(
				new StandardUtilities.StringCompare());
		final Set<JEditBuffer> buffers = new HashSet<JEditBuffer>();
		...
		// Wouter: we can not use the jEdit.visit() because it will iterate
		// over all View objects in the editor and we don't have View objects.
		// I think the idea here is that words from other views are also usable
		// as
		// candidates for the auto-completion in this view.
		// Instead we use the current buffer only.
		/*
		 * jEdit.visit(new JEditVisitorAdapter() {
		 * 
		 * @Override public void visit(EditPane editPane) { JEditBuffer b =
		 * editPane.getBuffer(); if(buffers.contains(b)) return;
		 * 
		 * buffers.add(b);
		 * 
		 * // only complete current buffer's keyword map KeywordMap _keywordMap;
		 * if(b == buffer) _keywordMap = keywordMap; else _keywordMap = null; //
		 * Wouter: _keywordMap is not used anywhere?!
		 * 
		...
		getCompletions(buffer, word, keywordMap, noWordSep, caret, completions);
	...
	// {{{ getCompletions() method
	/**
	 * Wouter: apparently this adds more completions to the given completions
	 * parameter
	 */
	private static void getCompletions(JEditBuffer buffer, String word,
		...
		for (int i = 0; i < buffer.getLineCount(); i++) {
			...
			for (int j = 0; j < len; j++) {
				...
				if (!Character.isLetterOrDigit(c) && noWordSep.indexOf(c) == -1) {
					if (StandardUtilities.regionMatches(line, j + 1, word, 0,
							wordLen) && caret != start + j + word.length() + 1) {
	...
	// {{{ completeWord() method
	private static String completeWord(CharSequence line, int offset,
			String noWordSep) {
	...
	private TextArea textArea; // Wouter: changed from jEditTextArea to textArea
	...
	private class Words implements Candidates {
		...
		public void complete(int index) {
			String insertion = completions[index].toString().substring(
					word.length());
			textArea.selectNone(); // deselect, see #1723
