...
class ModifiedBufferPrintable implements Printable {
	...
	ModifiedBufferPrintable(PrinterJob job, Object format, ViewSubstitute view,
		...
		styles = org.gjt.sp.util.SyntaxUtilities.loadStyles(
		...
		for (int i = 0; i < styles.length; i++) {
			...
			if (s.getForegroundColor().equals(Color.WHITE)
				...
				styles[i] = new SyntaxStyle(Color.BLACK,
						styles[i].getBackgroundColor(), styles[i].getFont());
			}
		}
	...
	private void printPage(Graphics grafix, PageFormat pageFormat,
		...
		if (lineNumbers) {
			...
			// and so on.
			int lineNumberDigits = (int) Math.ceil(Math.log(buffer
			...
			lineNumberWidth = font.getStringBounds(chars, 0, lineNumberDigits,
					frc).getWidth();
		} else {
			lineNumberWidth = 0.0;
		}
		...
		double tabWidth = font.getStringBounds(chars, 0, tabSize, frc)
				.getWidth();
		...
		print_loop: for (;;) {
			...
			if (!jEdit.getBooleanProperty("print.folds", true)
					&& !view.getTextArea().getDisplayManager()
							.isLineVisible(currentPhysicalLine)) {
	...
	private double paintHeader(Graphics2D gfx, double pageX, double pageY,
		...
		lm = font.getLineMetrics(headerText, frc1);

		Rectangle2D bounds = font.getStringBounds(headerText, frc1);
	...
	private double paintFooter(Graphics2D gfx, double pageX, double pageY,
		...
		// CHECK why use a new frc while there already is a global frc field?
		FontRenderContext frc1 = gfx.getFontRenderContext();
		lm = font.getLineMetrics(footerText, frc1);

		Rectangle2D bounds = font.getStringBounds(footerText, frc1);
		...
		if (actuallyPaint) {
			...
			gfx.drawString(footerText, (float) (pageX + (pageWidth - bounds
					.getWidth()) / 2),
					(float) (pageY + pageHeight - bounds.getHeight() + lm
							.getAscent()));
		}
