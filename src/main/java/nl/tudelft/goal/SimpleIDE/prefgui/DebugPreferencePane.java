...
public class DebugPreferencePane extends JPanel implements TableModelListener {
	...
	public DebugPreferencePane() {
		...
		// Create display.
		this.table = new JTable(new MyTableModel(channels));
...
class MyTableModel extends AbstractTableModel {
	...
	private String[] columnNames = { "Breakpoint", "View", "Pause" };
	...
	public MyTableModel(List<Channel> theChannels) {
		...
		getLongValues()[0] = new String(columnNames[0]);
		getLongValues()[1] = new String(columnNames[1]);
		getLongValues()[2] = new String(columnNames[2]);

		for (int i = 0; i < channels.size(); i++) {
			String expl = channels.get(i).getExplanation();
	...
	public Object getValueAt(int row, int col) {
		switch (col) {
		case 0:
			return channels.get(row).getExplanation();
		case 1:
			return isView(row);
		case 2:
			return isPause(row);
	...
	private Boolean isView(int row) {
		return DebugPreferences.getChannelState(channels.get(row)).canView();
	}

	private Boolean isPause(int row) {
		return DebugPreferences.getChannelState(channels.get(row))
				.shouldPause();
	}
	...
	public void setValueAt(Object val, int row, int col) {
		...
		switch (col) {
		case 1:
			DebugPreferences.setChannelState(channels.get(row),
